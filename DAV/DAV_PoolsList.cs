...
namespace Turbo.Plugins.DAV
{
	public class DAV_PoolsList : BasePlugin, IInGameTopPainter, IInGameWorldPainter, IAfterCollectHandler, IKeyEventHandler, INewAreaHandler {
		...
		public override void Load(IController hud) {
			...
			Key = SharpDX.DirectInput.Key.Z;
			xPos = 770; // Hud.Window.Size.Width * 0.4f;
			yPos = 10; // Hud.Window.Size.Height * 0.01f;
			iconSize = 80;
			showAlways = true;
			showALLPool = false; // summary show qty even if all pool used in that Act
			...
			PoolSumMessage = (act, qtyExist, qtyTotal) => "A" + act.ToString() + " : " + qtyExist.ToString() + " / " + qtyTotal.ToString();
